...
const FAQDemo = () => {
    ...
    const categoriesData = t.raw("categories");
    const faqDataa = t.raw("items");


    const categories = Object.fromEntries(
        categoriesData.map(({ key, label }) => [key, label])
    );



    console.log(faqDataa, "88");


    const faq = Object.fromEntries(
        faqDataa.map(({ category, questions }) => [category, questions.map(({ q, a }) => ({ question: q, answer: a }))])
    )
    // const categories = {
    ...
    return (
        <div className="mt-20 pb-20 bg-background">
            <FAQ
